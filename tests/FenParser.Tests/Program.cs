...
Check(Replay(ep).DeadPieces.Count == 0, "Imported position has no known captures");
Check(captured.DeadPieces.Count == 1 && captured.DeadPieces[0] is { Type: PieceType.Pawn, Color: PlayerColor.Black, DiedAtMove: 0 }, "En passant records captured pawn");
...
var captureGame = Read("Captures;2026-09-17;White;Black;E2-E4;D7-D5;E4-D5;");
var captureBoard = Replay(captureGame);
Check(captureBoard.DeadPieces.Count == 1 && captureBoard.DeadPieces[0] is { PieceId: 27, Type: PieceType.Pawn, Color: PlayerColor.Black, MoveCount: 1, DiedAtMove: 2 }, "Capture preserves piece identity and records death");
captureGame.Moves.RemoveAt(2);
Check(Replay(captureGame).DeadPieces.Count == 0, "Replaying before capture clears captured pieces");
Check(new BoardData().DeadPieces.Count == 0, "New game has no captures");
Check(promoted.DeadPieces.Count == 0 && castled.DeadPieces.Count == 0, "Promotion and castling are not captures");
